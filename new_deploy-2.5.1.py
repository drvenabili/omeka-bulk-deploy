...
class OmekaInstance:
    ...
    def extract_zip (self):
        z = zipfile.ZipFile ("omeka-2.5.1-reloaded.zip")
        z.extractall ()
        os.rename ("omeka-2.5.1", self.folder_name)
    ...
    def process_install_form (self):
        if (self.args.xvfb):
	    print "omeka"+self.folder_name
            display = Display(visible=0, size=(800, 600))
            display.start()
        driver = webdriver.Firefox()
        driver.get("http://localhost/" + self.folder_name + "/install")
...
class Deployer:
    ...
    def parse_args(self):
        ...
        parser.add_argument("--download-zip",
                            help="download omeka-1.5.3.zip from omekas's website !DEPRECATED!", # deprecated
                            action="store_true")
    ...
    def download_zip (self):
        os.system('wget -c http://omeka.org/files/omeka-1.5.3.zip') # deprecated
